...
The fine-tuned model (step-800, 500 datapoints) achieves an average prediction error of \textbf{16.51 runs}, an MSE of \textbf{543}, and an $R^2$ of \textbf{68.3\%}. An $R^2$ of 68.3\% indicates that the model accounts for approximately two-thirds of the variance in final first-innings totals across the test set, demonstrating that the natural-language prompt encodes substantial predictive signal that the fine-tuned model successfully exploits.
...
Despite being a substantially larger and more capable model by general-purpose benchmarks, \textit{GPT-4.1-nano} achieves poor predictive performance on this task, with an MAE of \textbf{30.02 runs} and an $R^2$ of only \textbf{14.0\%}. The low $R^2$ indicates that the model's predictions account for only a small fraction of the variance in first-innings totals, performing only marginally better than a naïve mean predictor.
