...
class Tester:
    ...
    def post_process(value):
        """Extract a numeric score from the model's raw output string."""
        if isinstance(value, str):
            value = value.strip()
            match = re.search(r"\b\d{2,3}\b", value)  # prefer 2-3 digit numbers (cricket scores)
            if match:
    ...
    def color_for(self, error, truth):
        """
        Green:  within 10 runs or 5% of actual score
        Orange: within 20 runs or 10% of actual score
        Red:    otherwise
        """
        if error < 10 or error / truth < 0.05:
            return "green"
        elif error < 20 or error / truth < 0.10:
            return "orange"
        else:
            return "red"
    ...
    def run_datapoint(self, i):
        ...
        # Build a short title from prompt fields: team + overs
        lines = datapoint["prompt"].split("\n")
        team, overs = "", ""
        for line in lines:
            if line.startswith("Batting team:"):
                team = line.replace("Batting team:", "").strip()
            elif line.startswith("Overs completed:"):
                overs = line.replace("Overs completed:", "").strip()
        title = f"{team} ({overs} ov)" if team else lines[0]
    ...
    def chart(self, title):
        ...
        fig = px.scatter(
            ...
            title=title,
            labels={"truth": "Actual Score (runs)", "guess": "Predicted Score (runs)"},
            width=800,
            height=600,
        )
    ...
    def error_trend_chart(self):
        ...
        final_ci = ci[-1]
        title = f"{self.title} — Avg Error: {final_mean:,.2f} ± {final_ci:,.2f} runs"
    ...
    def report(self):
        ...
        title = (
            ...
            f"<b>Avg Error:</b> {average_error:,.2f} runs  "
            f"<b>MSE:</b> {mse:,.0f}  "
            f"<b>r²:</b> {r2:.1f}%"
        )
    ...
    def run(self):
        for i in tqdm(range(self.size)):
            ...
            print(f"{COLOR_MAP[color]}{error:.0f} runs{RESET} ", end="")
        clear_output(wait=True)
        self.report()


def evaluate(predictor, data, size=DEFAULT_SIZE):
    Tester(predictor, data, size=size).run()